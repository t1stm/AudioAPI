...
def install(app: FastAPI, snapshot: Callable[[], Any], extra: APIRouter | None = None) -> None:
    """
    ...
    :param extra: a pod's own admin routes, nested under the same prefix and the same token check --
        the counterpart of hanging ``/Admin/library`` off what MapAdmin returns on the .NET side
        (Gaida.Pods.MusicDatabase/Program.cs). Passed in rather than returned because FastAPI copies a
        router's routes when it is included, so anything added after that point is never mapped.
        Unset ``ADMIN_TOKEN`` takes these down with the rest of the surface.
    """
