...
5. **Spotify can only resolve IDs.** `ContentDownloaders` is empty, so `spotify://` results can never play. It also needs `SPOTIFY_ID` / `SPOTIFY_SECRET`; unset, the platform now logs a warning and returns `null` instead of throwing at static-init time.
