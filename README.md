...
2. In the Developer Portal, enable Activities and add these **URL Mappings**:
   ...
   API URLs to `/.proxy/<prefix>` when it detects it's embedded
   (`src/lib/discord.ts`). Artwork lives on ytimg and the cover host, neither of
   which is mapped, so thumbnails are taken from `/Audio/Cover?id=` instead — the
   API fetches and caches them and serves them from its own origin.
